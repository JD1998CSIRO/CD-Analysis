...
def plot_cd_vs_wavelength(output_dir, pdf_pages, graph_titles, *csv_files):
    ...
    for i in range(num_pairs):
        ...
        export_file_path = os.path.join(output_dir, f"{output_filename_entry}_processed.csv")
        df_CD_blank_subtracted_filtered1.to_csv(export_file_path, columns=['Wavelength', 'Voltage', 'CD'], index=False)
    ...
    if num_pairs > 1:
        ...
        axes[0][num_pairs].set_ylabel('CD millidegrees')
    ...
    for i in range(num_pairs):
        _, df_additional1 = read_csv_until_string(csv_files[i * 2])
...
generate_pdf_button.grid(row=2, column=2, padx=10, pady=10)
